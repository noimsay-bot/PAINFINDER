...
export async function POST(request: Request) {
  try {
    ...
    const row = { name: config.name, is_default: false, model_stage1: config.model_stage1, model_stage2: config.model_stage2, model_verify: config.model_verify, mode_ratio: config.mode_ratio, families: config.families, domains: config.domains, excluded_domains: config.excluded_domains, sources: config.sources, period_days: config.period_days, app_list: config.app_list, limits: config.limits, updated_at: new Date().toISOString() };
